...
export default function DrinkCard({ item }) {
    ...
    const itemImage = item.image || 'https://via.placeholder.com/300x200?text=Starbucks';
    ...
    return (
        <div className="bg-white rounded-card shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col group">
            <Link to={`/menu/${itemId}`} className="relative block overflow-hidden">
                <img
                    src={itemImage}
                    alt={item.name}
                    className="w-full h-52 object-cover group-hover:scale-105 transition-transform duration-500"
                    onError={(e) => {
                        e.target.src = 'https://via.placeholder.com/300x200?text=Starbucks';
                    }}
                />
                {item.isNew && (
            ...
            <div className="p-4 flex flex-col flex-1">
                ...
                <p className="text-gray-500 text-sm flex-1 line-clamp-2 mb-3">{item.description || 'Delicious Starbucks item'}</p>
                <div className="flex items-center justify-between mt-auto">
                    <span className="text-sb-green font-bold text-lg">${itemPrice.toFixed(2)}</span>
                    <button
